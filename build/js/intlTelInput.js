 ...
 ********************/
    ...
    $.fn[pluginName].loadScript = function(path) {
        var utilsScript = path;
        if (utilsScript) {
            if (!$.fn[pluginName].loadedUtilsScript) {
                // don't do this twice! (dont just check if the global intlTelInputUtils exists as if init plugin multiple times in quick succession, it may not have finished loading yet)
                $.fn[pluginName].loadedUtilsScript = true;
    ...
    $.fn[pluginName].formatNumber = function(number) {
        ...
        scope.countryCodes = {};
        scope._addCountryCode = function(iso2, dialCode, priority) {
            if (!(dialCode in scope.countryCodes)) {
                scope.countryCodes[dialCode] = [];
            }
            var index = priority || 0;
            scope.countryCodes[dialCode][index] = iso2;
        };
        // generate countryCodes map
        for (var i = 0; i < scope.countries.length; i++) {
            var c = scope.countries[i];
            scope._addCountryCode(c.iso2, c.dialCode, c.priority);
            // area codes
            if (c.areaCodes) {
                for (var j = 0; j < c.areaCodes.length; j++) {
                    // full dial code is country code + dial code
                    scope._addCountryCode(c.iso2, c.dialCode + c.areaCodes[j]);
                }
            }
        }
        var dialCode;
        ...
        var countryCodes = scope.countryCodes[dialCode.replace(/\D/g, "")];
        for (var y = 0; y < countryCodes.length; y++) {
            if (countryCodes[y]) {
                countryCode = countryCodes[y];
                break;
            }
        }
        window.intlTelInputUtils.formatNumber(val, countryCode, null, true, true);
    };
